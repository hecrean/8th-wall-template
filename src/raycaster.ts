...
interface RaycasterApi {
  ...
  threeEvent: <K extends CanvasEventName>(
    ...
    raycaster,
  }: SceneGraphCtx) => ({ scene, camera }: RenderCxt) => IntersectionEvent<K>[];
}
  ...
  threeEvent:
    ...
    ({ scene, camera }: RenderCxt): IntersectionEvent<K>[] => {
      switch (canvasEvent.tag) {
        ...
        case "pointerover":
        case "pointerup": {
          canvasEvent.tag === "pointerenter"
            ? console.log(canvasEvent.tag)
            : () => {};
          ...
          const intersections: THREE.Intersection<
            THREE.Object3D<HTMLElementEventMap[K]>
          >[] = raycaster.intersectObjects(scene.children);

          const pointerEvts: IntersectionEvent<K>[] = intersections.map(
            (intersection) => ({
              ...intersection,
