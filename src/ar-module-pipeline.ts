...
type ShowTarget = {
  event: "model-target";
  process: (details: { name: "model-target" | "video-target" }) => void;
};
type HideTarget = {
  event: "model-target";
  process: (details: { name: "model-target" | "video-target" }) => void;
...
type CameraPipelineModule = {
  ...
  onUpdate?: (args: {
    ...
    processCpuResult: any;
  }) => void; // 	Called to update the scene before render. Data returned by modules in onProcessGpu and onProcessCpu will be present as processGpu.modulename and processCpu.modulename where the name is given by module.name = "modulename".
...
type XrController = {
  configure: (args: {
    enableWorldPoints: Optional<true>;
    enableLighting: Optional<true>;
    disableWorldTracking: Optional<true>;
    imageTargets: Optional<string[]>;
...
declare const XRExtras: XRExtras;
...
type SceneState = {
  ...
  videoObj: THREE.Object3D;
  videoEl: HTMLVideoElement;
};

type SceneUpdate = (state: SceneState) => ({ detail }: { detail: any }) => void;
type SceneUpdates = { [key: string]: SceneUpdate };
  ...
  showTarget:
    ...
    ({ detail }) => {
      ...
      if (detail.name === "model-target") {
        state.model.position.copy(detail.position);
        state.model.quaternion.copy(detail.rotation);
        state.model.scale.set(detail.scale, detail.scale, detail.scale);
        state.model.visible = true;
      }
      // When the image target named 'video-target' is detected, play video.
      // This string must match the name of the image target uploaded to 8th Wall.
      if (detail.name === "video-target") {
        state.videoObj.position.copy(detail.position);
        state.videoObj.quaternion.copy(detail.rotation);
        state.videoObj.scale.set(detail.scale, detail.scale, detail.scale);
        state.videoObj.visible = true;
        state.videoEl.play();
      }
    },
  hideTarget:
